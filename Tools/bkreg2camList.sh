...
do
	iconv -f UTF-16LE -t UTF-8  ${host}.reg | grep -E "\"enabled\"|\"group\"|shortname|dsname|fullxres|fullyres|&value=3|\"vcodec\"|\"folder\"" | sed -e "s|10.10.1.45/JSON?user=rpi&pass=this1ok&request=controldevicebyvalue&ref=||g" | sed -e "s|&value=3||g" > ${host}.raw
	echo ======== Raw =========
	cat ${host}.raw
	echo ======== Raw =========
